...
{
  static readonly Action<Structure> AddStructure;
  static readonly Action<Structure> RemoveStructure;

  static GridControllerPatch()
  {
    if (typeof(GridController).GetField("AllStructures", BindingFlags.Public | BindingFlags.Static)?.GetValue(null) is List<Structure> allStructures)
    {
...
static class XmlSaveLoadPatch
{
  ...
  private static readonly AddToSaveDelegate AddToSave;
...
static class NetworkServerPatch
{
  ...
  private static readonly Stack<PlacementBoard> SerializingBoards = new();
